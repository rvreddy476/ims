...
namespace IMS.Web.Controllers
{
	...
	public class LoginController : Controller
	{
		...
		[HttpGet]
		public ActionResult Login(bool sessionExpired=false )
		{
			if (sessionExpired)
			{
			...
			try
			{
				...
				if (isLDAP) //available in LDAP
				{
					...
					if (isValidUser == true)
					{
						...
						return Json(new { data = "10" });
					}

				}
				else if (isValidUser == true)//available in htv
				{
		...
		public string ForgotPassword(IMSEntity model)
		{
			...
			try
			{
				...
				using (var writer = new StreamWriter(@"E:\IMSDocs\logfile.txt"))
				{
