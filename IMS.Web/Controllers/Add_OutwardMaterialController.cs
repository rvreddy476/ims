    ...
    public class Add_OutwardMaterialController : Controller
    {
        ...
        public ActionResult OutwardList(string sortOrder, string searchString, string Name, string SearchValue1, string SearchValue2, string SearchValue, string SearchValue_ExpenseN, FormCollection form, DateTime? searchdate, int? Page_no)
        {
            ...
            if (role != null)
            {
                ...
                ViewBag.OW_MaterialID = String.IsNullOrEmpty(sortOrder) ? "Inward_id_desc" : "";
                ViewBag.OW_department = sortOrder == "OW_department" ? "OW_department_desc" : "OW_department";
                ViewBag.OW_Location = sortOrder == "OW_Location" ? "OW_Location_desc" : "OW_Location";
                ViewBag.Outward_datetime = sortOrder == "Outward_datetime" ? "Outward_datetime_desc" : "Outward_datetime";
                ViewBag.Outward_Status = sortOrder == "Outward_Status" ? "Outward_Status_desc" : "Outward_Status";
                ...
                try
                {
                    ...
                    {
                        case "OW_MaterialID":
                            OutwardDetails = OutwardDetails.OrderByDescending(p => p.OW_MaterialID);
                            break;

                        case "OW_department":
                            OutwardDetails = OutwardDetails.OrderByDescending(p => p.userDepartmentName);
                            break;

                        case "OW_department_desc":
                            OutwardDetails = OutwardDetails.OrderBy(p => p.userDepartmentName);
                            break;

                        case "Outward_Location":
                            OutwardDetails = OutwardDetails.OrderByDescending(u => u.Location);
                            break;

                        case "Outward_Location_desc":
                            OutwardDetails = OutwardDetails.OrderBy(u => u.Location);
                            break;
                        ...
                        default:
                            OutwardDetails = OutwardDetails.OrderByDescending(p => p.OutwardDatetime);
                            break;
