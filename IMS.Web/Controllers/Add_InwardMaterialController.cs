...
namespace IMS.Web.Controllers
{
	public class Add_InwardMaterialController : Controller
	{
		...
		public bool generatedInwardMaterial(IMSEntity model1, string finaldata)
		{
			...
			var OutputPath = @"E:\IMSDocs\InwardMaterial\" + model.InwardID;
			...
			{
				string[] oldfilename = Directory.GetFiles(@"E:\IMSDocs\InwardMaterial\" + model.InwardID);
			...
			var imagepath = @"E:\IMSDocs\AllWebSites\";
		...
		public bool UpdateInwardPDF(string InwardId)
		{
			try
			{
				...
				var httpCookie2 = HttpContext.Session["UserID"];
				if (httpCookie2 != null)
				{
				...
				var OutputPath = @"E:\IMSDocs\InwardMaterial\" + InwardId;
			...
			try
			{
				...
				var materialdetails = (from t in context.Material_Category
									   ...
									   select new IMSEntity
									   {
										   ...
										   Material_Remark = i.Material_Remark,
										   MaterialName = i.MaterialName,
										   Inward_Type = i.Inward_Type,
										   Quantity = i.Quantity,
										   InvMaterialTemp_ID = i.InvMaterialTemp_ID,
										   Material_CategoryID = t.Material_CategoryID,
										   Material_CategoryName = t.Material_CategoryName
									   }).ToList();

				vmsentity = materialdetails;
				ViewBag.rowlength = materialdetails.Count();
				imsentity.IMSEntities = materialdetails;
		...
		public ActionResult Edit_InwardMaterial(IMSEntity model, HttpPostedFileBase[] DC_Filename, HttpPostedFileBase DC_Filename_EXI, IEnumerable<IMSEntity> editedRows)
		{
			...
			try
			{
				...
				{
					DCid = unitOfWork.DepartmentRepository.GenerateInwardIDS("Deliverychallen");
					file_value_extra = unitOfWork.DepartmentRepository.filename(DC_Filename[0], "Delivery Challen", DCid, null);
		...
		public bool sendInwardMail(string id, string type)
		{
			try
			{
				...
				{
					DirectoryInfo dir = new DirectoryInfo(@"E:\IMSDocs\DeliveryChallen\" + id + "\\" + f.DC_ID);
				...
				if (type == "Inward Added")
				{
					...
					var regards = "<br/>Best Regards <br/> Team Harbinger IMS";
					Body = Body + regards;
				...
				else if (type == "Inward Rejected")
				{
					...
					Body = "I hope this email finds you well. This is to inform you that the following materials have been rejected: Please find the details below:" +
						   ...
						   + "<table border = 1> <tr><th>srno.</th><th>Material Name</th><th>Qty</th><th> Inward Type </th>" + materialhtml
						   + "<br/>Best Regards <br/> Team Harbinger IMS";
		...
		public ActionResult Delete_InwardMaterial(string id)
		{
			if (Session["Email"] != null)
			{
				try
				{
					...
					// Find the InwardMaterial and DeliveryChallens by ID
					InwardMaterial inward_delete = context.InwardMaterials.Single(x => x.InwardID == id);
					var deliveryChallen = context.DeliveryChallens.Where(x => x.InwardID == id);

					// Delete InwardMaterial from the database
					unitOfWork.DepartmentRepository.Delete(inward_delete);
					...
					// Save changes to commit the deletion in the database
					unitOfWork.Save();
				}
				catch (Exception e)
				{
					...
					// Log the exception in the IMSExceptionLogger
					var exceptionLogger = new IMSExceptionLogger()
					{
						controllerName = "Add_InwardMaterialController",
						actionName = "Delete_InwardMaterial",
		...
		public bool sendGRNNumber(string grnnumber, string inwardid, string status, string vendorid, string expensenature)
		{
			try
			{
				...
				foreach (var f in deliverchallen)
				{
					DirectoryInfo dir = new DirectoryInfo(@"E:\IMSDocs\DeliveryChallen\" + inwardid + "\\" + f.DC_ID);
					foreach (FileInfo file2 in dir.GetFiles("*.*"))
					{
						if (file2.Exists)
						{
		...
		public ActionResult ViewInwarddocx(string id, string name, string InvId)
		{
			if (Session["Email"] != null)
			{
				...
				try
				{
					...
					{
						filesNames1 = Directory.GetFiles(@"E:\IMSDocs\DeliveryChallen\" + InvId + "\\" + id);
					}
					...
					if (name == "DeliveryChallen")
					{
						filePath = @"E:\IMSDocs\DeliveryChallen\" + InvId + "\\" + id + "\\" + fileName;
					}
		...
		public ActionResult DownloadInwarddocx(string items, string InvId, string type, string dcid)
		{
			if (Session["Email"] != null)
			{
				...
				try
				{
					...
					{
						string FileVirtualPath = @"E:\IMSDocs\DeliveryChallen\" + InvId + "\\" + dcid + "\\" + items;
						return File(FileVirtualPath, "application/force-download", Path.GetFileName(FileVirtualPath));
					}
					if (type == "InwardPDF")
					{
						string FileVirtualPath = @"E:\IMSDocs\InwardMaterial\" + InvId + "\\" + items;
						return File(FileVirtualPath, "application/force-download", Path.GetFileName(FileVirtualPath));
					}
					else if (type == "Invoice")
					{
						string FileVirtualPath = @"E:\HTV-VMSDocs\Invoice\" + InvId + "/" + items;
						return File(FileVirtualPath, "application/force-download", Path.GetFileName(FileVirtualPath));
					}
		...
		public ActionResult ViewInvoicedocx(string id, string name)
		{
			...
			try
			{
				...
				string[] filesNames1 = null;
				if (name == "Invoice")
					filesNames1 = Directory.GetFiles(@"E:\HTV-VMSDocs\Invoices\" + id + " / ");
				else
					filesNames1 = Directory.GetFiles(@"E:\HTV-VMSDocs\InvoiceSupportDoc\" + id + " / ");
				...
				if (name == "Invoice")
					filePath = @"E:\HTV-VMSDocs\Invoices\" + id + "/" + fileName;
				else
					filePath = @"E:\HTV-VMSDocs\InvoiceSupportDoc\" + id + "/" + fileName;
				Response.AddHeader("Content-Disposition", "inline; filename=" + fileName);
		...
		[HttpGet]
		public JsonResult ValidateMaterialWithCategory(string inwardId, string materialID, int categoryId)
		{
			var materialCount = (from t in context.Material_Category
								 join c in context.Materials on t.Material_CategoryID equals c.Material_CategoryID
								 join i in context.InwardMaterial_Temp on c.MaterialID equals i.MaterialID
								 where i.InwardID.Equals(inwardId) && c.MaterialID.Equals(materialID)
								 select c).AsQueryable().Count();


			return Json(new { isExists = materialCount > 0 ? true : false }, JsonRequestBehavior.AllowGet);
		}
	}
}