...
namespace IMS.DAL.Infrastructure
{
    public abstract class RepositoryBase<TEntity> where TEntity : class
    {
                ...
                if (Docfile.ContentLength > 0)
                {
                    ...
                    switch (Name)
                    {
                        ...
                        case "Delivery Challen":
                            path = Path.Combine(@"E:\IMSDocs\DeliveryChallen\" + inwardid );
                            path = path + "\\" + id;
                            break;

                        default:
                            path = Path.Combine(@"E:\HTV-VMSDocs\Po\", newFileName);
                            break;
                    }
                    ...
                    if (Name == "Supportingdocx")
                    {
                        ...
                        //var path_deleted = Path.Combine(@"E:\HTV-VMSDocs\Supportingdocx\Deleted", id);
                        ...
                        //    StrFilePathName = path + "\\" + filename;
                        //}
                        //StrFilePathName = "E:\\test" + count + ".csv";
        ...
        public List<IMSEntity> GetFileDetails(string id, string categoryName)
        {
            ...
            var path = Path.Combine(@"E:\IMSDocs\" + cat_name + "\\", id);
            string[] oldfilename = Directory.GetFiles(path);
            List<string> items = new List<string>();
        ...
        public void DeletedFiles(string Name, string id, string InwardId)
        {
            try
            {
                ...
                {                  
                    path = Path.Combine(@"E:\IMSDocs\DeliveryChallen\" + InwardId + "\\", id);
                    desination = Path.Combine(@"E:\IMSDocs\DeliveryChallen\", "Deleted");
                }

                if(Name == "InwardMaterial")
                {
                    path = Path.Combine(@"E:\IMSDocs\InwardMaterial\", InwardId);
                    desination = Path.Combine(@"E:\IMSDocs\InwardMaterial\", "Deleted");
                }
